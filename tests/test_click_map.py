...
class ClickMapTest(unittest.TestCase):
    def test_write_click_map_embeds_screenshot_and_outputs_helpers(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn('class="workflow-rail"', content)
            self.assertIn('id="deviceInputState"', content)
            self.assertIn('id="serialState"', content)
            self.assertIn('id="adbState"', content)
