...
尚未完成的是自動決策。AutoPlay 目前不會自行理解遊戲畫面並任意點擊；下一階段會先做 guided gesture calibration，再用 checkpoint/template match 建立 bounded decision loop。
...
`calibration write/show` 是目前已完成的 profile 基礎。下一階段的 `calibration guide` 會把「測一次、回報太短/剛好/太長、產生 profile 與 notes」整理成互動流程，避免手動猜距離與編輯 JSON。
...
請從 `docs/next-stage.md` 與 `docs/specs/0020-guided-gesture-calibration.md` 接續：

1. 實作 CLI-first 的 `py -m autoplay calibration guide`。
2. 讓測試者不用手改 JSON 就能校準 scroll distance、screen dimensions 與 notes。
